...
data(diamonds)
library(ggplot2)
str(diamonds) #see what type of data there are. While the other titles are clear, I do not know what x,y and z are supposed to represent.


#1. Extracts all numeric columns from the data frame.
num<- sapply(diamonds, is.numeric)
num #assigns the columns with numeric data to the variable num. Does this by applying the function is.numeric to diamonds dataframe to get all of the numeric columns. This tests whether they are numeric or not and returns TRUE or FALSE. It then assigns to num all the TRUE columns. Use sapply 
diamonds[ ,num] #this prints the columns of the dataframe diamonds with numeric columns (num) and only those. By leaving the "rows" part of the call blank, this prints out all row values for that column. This structure could also be used to call a specific point in the dataframe, say the 2nd row of the 3rd column. 

#By using these three lines and subsituting in the name of the dataframe, you could extract the numeric columns from any dataframe. If you wanted to use this information later, all you would have to do would be to assign diamonds[ ,num] to a variable and it would be the dataframe of all numeric values.


#2/3.
g<-diamonds[ ,num] #As mentioned above, assigns the dataframe with all numeric columns of diamonds to the variable g
d<-names(g) #assigns d to be the column names of g
...
#These next two lines are #3, the graphing. It is easiest to contain them in the same while loop, because we want to graph them all seperately. But the rest of the while loop is still necessary for question 2
  u<-c(h, z)
  plot(g[ ,p[i]], g[ ,p[j]], xlab=d[p[i]], ylab=d[p[j]],main=u)
